...
describe("hello-world MPC circuit", () => {
  ...
  const compDefOffset = Buffer.from(getCompDefAccOffset("hello_world")).readUInt32LE(0);
  const compDefAccountAddress = getCompDefAccAddress(program.programId, compDefOffset);
  ...
  it("initializes hello_world computation definition", async () => {
    try {
      ...
      if (err.toString().includes("already in use") || err.toString().includes("initialized")) {
  ...
  it("queues hello_world computation and awaits finalization", async () => {
    ...
    console.log("  Computation account:", computationAccountAddress.toBase58());
    ...
    const hasResult = logs.some(
      (log) =>
        log.includes("Hello World MPC result") || log.includes("hello_world_callback")
    );
    assert.isTrue(hasResult, "Callback logs should contain hello_world result");
