...
(RUN_ARCIUM_INTEGRATION_TESTS ? describe : describe.skip)(
  ...
  () => {
    ...
    before(async () => {
      ...
      const { awaitComputationFinalization } = await import(
        "@arcium-hq/client"
      );
    ...
    describe("resolver pool", () => {
      it("registers resolver with 500 USDC minimum", async () => {
        ...
        // Verify Resolver PDA
        const resolverAccount = await program.account.resolver.fetch(
          resolverPda
        );
        assert.ok(
        ...
        assert.isFalse(resolverAccount.approved, "Should not be approved yet");
        assert.equal(
      ...
      it("resolver can top up stake", async () => {
        ...
        const resolverBefore = await program.account.resolver.fetch(
          resolverPda
        );
        ...
        try {
          ...
          assert.isTrue(
            errStr.includes("NotMarketParticipant") ||
              ...
              errStr.includes("Error"),
            `Expected NotMarketParticipant error, got: ${errStr.substring(
              0,
              300
            )}`
          );
    ...
    describe("encrypted voting (MPC-dependent)", () => {
      ...
      it("init_dispute_tally instruction exists with correct accounts in IDL", async () => {
        ...
        assert.isDefined(
          ix,
          "initDisputeTally instruction should exist in IDL"
        );
      ...
      it("cast_vote instruction exists with correct accounts in IDL", async () => {
        ...
        assert.isTrue(argNames.includes("pubKey"), "Should have pubKey arg");
        assert.isTrue(argNames.includes("nonce"), "Should have nonce arg");

        console.log(
          "    castVote instruction verified in IDL with correct args"
        );
        console.log("    TODO: Execute cast_vote MPC when DKG is operational");
      });
      ...
      it("NotSelectedJuror error variant exists for juror eligibility enforcement", async () => {
        ...
        console.log(
          `    NotSelectedJuror error verified: code=${error!.code}, msg="${
            error!.msg
          }"`
        );
      });
      ...
      it("AlreadyVoted error variant exists for one-vote enforcement", async () => {
        const idlErrors = program.idl.errors;
        const error = idlErrors!.find(
          (e: any) => e.name === "alreadyVoted" || e.name === "AlreadyVoted"
        );
        assert.isDefined(
          error,
        ...
        console.log(
          `    AlreadyVoted error verified: code=${error!.code}, msg="${
            error!.msg
          }"`
        );
      });
      ...
      it("VotingWindowClosed error variant exists for time enforcement", async () => {
        ...
        console.log(
          `    VotingWindowClosed error verified: code=${error!.code}, msg="${
            error!.msg
          }"`
        );
      });
      ...
      it("DisputeNotVoting error variant exists for state enforcement", async () => {
        ...
        console.log(
          `    DisputeNotVoting error verified: code=${error!.code}, msg="${
            error!.msg
          }"`
        );
      });
    ...
    describe("dispute finalization (MPC-dependent)", () => {
      it("finalize_dispute instruction exists with correct accounts in IDL", async () => {
        ...
        assert.isDefined(ix, "finalizeDispute instruction should exist in IDL");
      ...
      it("QuorumNotReached error variant exists for quorum enforcement", async () => {
        ...
        console.log(
          `    QuorumNotReached error verified: code=${error!.code}, msg="${
            error!.msg
          }"`
        );
      });
      ...
      it("add_tiebreaker instruction exists with correct accounts in IDL", async () => {
        ...
        assert.isDefined(ix, "addTiebreaker instruction should exist in IDL");
      ...
      it("TiebreakerAlreadyAdded error variant exists", async () => {
        ...
        console.log(
          `    TiebreakerAlreadyAdded error verified: code=${
            error!.code
          }, msg="${error!.msg}"`
        );
      });
      ...
      it("DisputeNotSettled error variant exists for settlement enforcement", async () => {
        ...
        console.log(
          `    DisputeNotSettled error verified: code=${error!.code}, msg="${
            error!.msg
          }"`
        );
      });
      ...
      it("MarketAlreadyDisputed error variant exists", async () => {
        ...
        console.log(
          `    MarketAlreadyDisputed error verified: code=${
            error!.code
          }, msg="${error!.msg}"`
        );
      });
    ...
    describe("dispute lifecycle structure", () => {
      it("Dispute account type exists in IDL with correct fields", async () => {
        ...
        assert.isDefined(
          disputeType,
          "Dispute account type should exist in IDL"
        );
      ...
      it("DisputeTally account type exists in IDL with correct fields", async () => {
        const disputeTallyType = program.idl.accounts?.find(
          (a: any) => a.name === "disputeTally" || a.name === "DisputeTally"
        );
        assert.isDefined(
          disputeTallyType,
