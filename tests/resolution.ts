...
(RUN_ARCIUM_INTEGRATION_TESTS ? describe : describe.skip)(
  ...
  () => {
    ...
    it("places bets for resolution testing (User A: 10 USDC Yes, User B: 5 USDC No)", async () => {
      ...
      assert.equal(
        posA.yesAmount.toNumber(),
        betAmountA,
        "User A yes_amount should be 10 USDC"
      );
      ...
      assert.equal(
        posB.yesAmount.toNumber(),
        0,
        "User B yes_amount should be 0"
      );
      assert.equal(
        posB.noAmount.toNumber(),
        betAmountB,
        "User B no_amount should be 5 USDC"
      );
    ...
    it("rejects resolve_market before deadline (MarketNotExpired)", async () => {
      ...
      try {
        ...
        console.log(
          "    MarketNotExpired correctly rejected for future-deadline market"
        );
      }
    });
    ...
    it("rejects resolve_market with invalid outcome (InvalidOutcome)", async () => {
      try {
        ...
        console.log(
          "    InvalidOutcome correctly rejected (winning_outcome=3)"
        );
      }
    });
    ...
    it("resolves market with winning_outcome=1 (Yes wins)", async () => {
      ...
      assert.equal(
        ...
        1,
        "Winning outcome should be 1 (Yes)"
      );
    ...
    it("computes payouts via MPC and transitions to Finalized (state=4)", async () => {
      const computeOffset = encryptBetInput(
        arciumCtx.mxePublicKey,
        true,
        0
      ).computationOffset;
    ...
    it("pays winner (User A) correct proportional payout with fee deducted", async () => {
      ...
      const userABalanceBefore = (
        await getAccount(connection, userATokenAccount)
      ).amount;
      const feeBalanceBefore = (
        await getAccount(connection, feeRecipientTokenAccount)
      ).amount;
      ...
      assert.equal(
        userAGain,
        14_700_000,
        "User A should receive 14.7 USDC net payout"
      );
      ...
      assert.equal(
        feeGain,
        300_000,
        "Fee recipient should receive 0.3 USDC fee"
      );
      ...
      assert.isTrue(
        position.claimed,
        "User A position should be marked as claimed"
      );
    ...
    it("rejects loser claim (User B bet No, Yes won) with NoWinningPosition", async () => {
      ...
      try {
        ...
        console.log(
          "    NoWinningPosition correctly rejected (User B bet No, Yes won)"
        );
      }
    });
    ...
    it("rejects double claim (User A already claimed) with AlreadyClaimed", async () => {
      ...
      try {
        ...
        console.log(
          "    AlreadyClaimed correctly rejected (User A already claimed)"
        );
      }
    });
  }
