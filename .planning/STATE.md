...
**Recent Trend:**
- Last 5 plans: 2min, 3min, 1min, 4min, 2min
- Trend: Phase 10 complete -- RTG submission documentation done, all v1.0 milestones achieved
...
| Phase 10-rtg-submission P02 | 2min | 2 tasks | 1 files |
| Phase 10-rtg-submission P01 | 2min | 2 tasks | 3 files |
...
- [10-01]: Avenir Contributors as copyright holder for MIT license (not individual name)
- [10-01]: artifacts/trusted_dealer_config.toml private_key_path confirmed as path reference false positive
- [10-02]: Problem-first narrative: lead with Keynesian beauty contest herding problem before revealing Arcium solution
- [10-02]: Two Mermaid diagrams: system overview (TB layout) + encrypted betting flow (LR layout) with green/gray/purple color coding
- [10-02]: update_pool circuit as primary code highlight with annotated Enc types and reveal semantics
- [10-02]: Fog UX section with 4 screenshot references (images pending capture at checkpoint)
- [10-02]: Screenshots deferred to user -- checkpoint approved, user will capture fog UX screenshots separately
- [13-01]: crypto-browserify Vite alias to unblock @arcium-hq/client browser bundling instead of SDK upgrade
- [13-01]: Combined cipher.encrypt([isYes, amount], nonce) call fixes CTR counter-reuse bug from split calls
- [13-01]: createReadonlyAnchorProvider extracted from useReadOnlyProgram for non-hook MXE key lookup reuse
- [13-01]: Minimal process polyfill shim in client.tsx for crypto-browserify compatibility
- [13-02]: Single-field vote encryption cipher.encrypt([isYes], nonce) -- intentionally simpler than bet's combined two-field call
- [13-02]: JurorVotePanel required zero changes -- hook return shape { castVote, loading, error } preserved exactly
- [13-03]: Server encryption module replaced with guard (not deleted) -- provides runtime error if accidentally re-imported
- [13-03]: Fast SDK tests use locally generated x25519 keys -- proves cipher contract without DKG/Arcium runtime
- [13-03]: MPC integration tests gated behind ARCIUM_INTEGRATION_TESTS env flag to separate fast from live tests
...
- [16-01]: Secrets scan false positives confirmed: sharedSecret/privateKey in client-encryption.ts are variable names, not secrets
- [16-01]: Repository made public via gh repo edit --visibility public; RTG-01 closed
...
- User to capture 3-4 fog UX screenshots in docs/screenshots/ (market-feed-fog.png, encrypted-sentiment.png, fog-reveal-resolved.png, dispute-fog.png)
