  ...
  const mutation = useMutation({
    mutationFn: async (input: CreateMarketInput) => {
      ...
      if (!marketCreated) {
        ...
        const marketCounter = (config as any).marketCounter;
        const current =
          typeof marketCounter === "number"
            ? marketCounter
            : marketCounter?.toNumber?.() ?? Number(marketCounter);
        const createdMarketId = current + 1;
        nextMarketId = createdMarketId;
        setNewMarketId(createdMarketId);

        // Derive PDAs for the new market
        const [marketPda] = getMarketPda(createdMarketId);
        const [marketPoolPda] = getMarketPoolPda(createdMarketId);
        const [marketVaultPda] = getVaultPda(createdMarketId);
    ...
    onError: (error) => {
      ...
      if (marketCreated) {
        ...
        toast.error("Market created but pool initialization failed. Retry?", {
          action: {
            label: "Retry",
            onClick: () => {
              setStep("idle");
